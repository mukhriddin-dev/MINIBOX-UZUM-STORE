    ...
    return (
        <div className="flex items-center border-t-[1px] py-[20px] justify-between">
                    ...
                    <div className="flex items-start justify-between min-w-[450px] w-full  h-[65px]">
                        <div className="flex flex-col justify-end  h-[65px]">
                            <p className="text-gray-600 text-[13px]">
                                Ja'mi qolgan: <span className="text-black font-semibold">{item?.count}</span> ta
                            </p>
                            <p className="text-gray-600 text-[13px]">
                                Sotuvchi: <span className="text-black font-semibold">Sunnatbek</span>
                            </p>
            ...
            <div className="flex flex-col items-end">
                <button className="p-1 my-2 focus:bg-red-200 border" onClick={() => deleteCart(item.id)}>
                    O'chirish
                </button>
                <p className="text-white bg-green-500 px-1 text-[13px] mb-3 rounded-md">10% chegirma</p>
                <h1 className="text-[18px]">
                    <strong>
                        {String(item?.price * count - item?.price * count * 0.1).replace(/\B(?=(\d{3})+(?!\d))/g, ".")}{" "}
                    </strong>
                    so'm
                </h1>
                <del className="text-gray-500 text-[15px]">
                    {String(item?.price * count).replace(/\B(?=(\d{3})+(?!\d))/g, ".")} so'm
                </del>
            </div>
        </div>
    );
